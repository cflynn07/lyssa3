...
module.exports = (function() {
  ...
  QueryInterface.prototype.select = function(factory, tableName, options, queryOptions) {
    ...
    // sql = "SELECT `employees`.*, `templates`.`uid` AS `templates.uid`, `templates`.`id` AS `templates.id`, `templates`.`name` AS `templates.name`, `templates`.`type` AS `templates.type`, `templates`.`clientUid` AS `templates.clientUid`, `templates`.`employeeUid` AS `templates.employeeUid`, `templates`.`createdAt` AS `templates.createdAt`, `templates`.`updatedAt` AS `templates.updatedAt`, `templates`.`deletedAt` AS `templates.deletedAt`, `templates`.`clientId` AS `templates.clientId`, `templates`.`employeeId` AS `templates.employeeId` FROM `employees` LEFT OUTER JOIN `templates` AS `templates` ON `employees`.`id` = `templates`.`employeeId` WHERE `employees`.`clientUid`='44cc27a5-af8b-412f-855a-57c8205d86f5' and employees.id=2;"
